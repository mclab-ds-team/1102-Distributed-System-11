...
def send_kafka(partition, id, pos, food1, food2, food3):
    data = {
    ...
    producer.send(ORDER_KAFKA_TOPIC, json.dumps(data).encode("utf-8"), partition=partition)
...
def submit():
    ...
    if request.method == 'POST':
        if request.values['Submit'] == '確認送出':
            bottom_pross += 1
            bottom_pross = bottom_pross % 2
            user_id = request.values['user_id']
            position = int(request.values['p1'])
            food_burger = int(request.values['burger'])
            food_sandwich = int(request.values['sandwich'])
            food_cola = int(request.values['cola'])
            send_kafka(bottom_pross, user_id, position, food_burger, food_sandwich, food_cola)
    return render_template('home.html')
